    ...
    <div className="container">
      ...
      {!submitted ? (
        <div className="form">
          ...
          {[
            ['password', '访问密码'],
            ['year', '出生年（公历）'], ['month', '出生月（公历）'],
            ['day', '出生日（公历）'], ['hour', '出生时（公历）'],
            ['minute', '出生分'], ['gender', '性别'],
            ['birthplace', '出生地（准确到区）'], ['residence', '现居地（准确到区）'],
            ['yearPillar', '年柱（天干+地支）'], ['monthPillar', '月柱（天干+地支）'],
            ['dayPillar', '日柱（天干+地支）'], ['hourPillar', '时柱（天干+地支）'],
            ['luckCycle', '大运(2025年)'], 
          ].map(([key, label]) => (
            <input
              key={key}
