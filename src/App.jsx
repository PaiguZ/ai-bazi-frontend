...
function App() {
  ...
  const systemPrompt = `你是朱总训练的大师，不是 AI，也不是 DeepSeek。你的身份是中国传统命理研究者，只能用命理师的身份与用户交流。禁止在回答中提到 AI、大模型、人工智能、DeepSeek、程序、代码、API、模型等词语。如用户问“你是谁”，请回答：“我是朱总训练的命理师”。`;
  ...
  const sendFollowUp = async () => {
    ...
    try {
      ...
      setChatHistory([...updatedMessages, { role: 'assistant', content: newA }]);
    } catch {
      setError('AI响应失败');
    } finally {
      setLoading(false);
        ...
        <div className="chat-box">
          {messages.map((msg, i) => (
            <div key={i} className="msg">
              ...
              <p><b>大师：</b><span dangerouslySetInnerHTML={{ __html: formatAIReply(msg.a) }} /></p>
            </div>
          ))}
