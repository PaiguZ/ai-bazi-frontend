...
function App() {
  ...
  const submitInfo = async () => {
    ...
    try {
      ...
      setMessages([...initialMessages, { role: 'assistant', content: res.data.reply }]);
    } catch (e) {
      setError('请求失败，请稍后重试。');
    } finally {
      setLoading(false);
        ...
        <div className="chat-box">
          {messages.map((msg, i) => (
            <div key={i} className="msg">
              {msg.role === 'user' && <p><b>你：</b>{msg.content}</p>}
              {msg.role === 'assistant' && (
                <p><b>AI：</b><span dangerouslySetInnerHTML={{ __html: formatAIReply(msg.content) }} /></p>
              )}
            </div>
          ))}
